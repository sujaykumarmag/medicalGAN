...
model = torch.load("models/Pneu-ViT.pth")
...
for batch in val_loader:
    ...
    tot_preds.append(preds.numpy())
    tot_labels.append(labels.numpy())
    acc = acc + accuracy_score(preds.numpy(),labels.numpy())
    #print(classification_report(preds.numpy(),labels.numpy()))
    i = i+1



print("-------------------Accuracy By Aggregation-------------------")
print(acc/i)

tot_labels = np.array(tot_labels).reshape(-1)
tot_preds = np.array(tot_preds).reshape(-1)
print(classification_report(tot_preds,tot_labels))   
print(confusion_matrix(tot_preds,tot_labels)) 
